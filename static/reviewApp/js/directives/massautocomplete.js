angular.module('MassAutoComplete', [])
    .directive('massAutocomplete', ["$timeout", "$window", "$document", "$q", function ($timeout, $window, $document, $q) {
      ...
      return {
        ...
        '<div class="suggestion-menu-position ac-container" ng-show="show_autocomplete && results.length > 0">' +
        '<ul class="ac-menu">' +
        '<li ng-repeat="result in results" ng-if="$index > 0" ' +
        'class="ac-menu-item suggestion-item-top" ng-class="$index == selected_index ? \'ac-state-focus\': \'\'">' +
        '<span ng-click="apply_selection($index)"><a href  class="suggestion-name-position" ng-bind-html="result.name"></a>' +
        '<a href class="suggestion-type-position" ng-bind-html="result.type"></a></span>' +
        '</li>' +
        '</ul>' +
        '</div>',
        ...
        controller: ["$scope", "$rootScope", function ($scope, $rootScope) {
              ...
              $q.when(current_options.suggest(term),
                  function suggest_succeeded(suggestions) {
                    ...
                    if (suggestions && suggestions.length > 0) {
                      ...
                      $scope.results = [{ name: term}].concat(suggestions);
          ...
          function set_selection(i) {
            ...
            current_element.val(selected.name);
          ...
          $scope.apply_selection = function (i) {
            ...
            if (!$scope.show_autocomplete || i > $scope.results.length || i < 0)
              return;
            $rootScope.setSelectedRecordData($scope.results[i]);
            var selected = set_selection(i);
            last_selected_value = selected.name;
            update_model_value(selected.name);
            $scope.show_autocomplete = false;
              ...
              switch (e.keyCode) {
                ...
                case KEYS.DOWN:
                  if ($scope.results.length > 0) {
                    if ($scope.show_autocomplete) {
                      console.log('selected index: ',$scope.selected_index );
